...
function getHexadecimalValue(data, i) {
	...
	var z = 2;
	while(true) {
		if(data[i+z] === 1 || data[i+z] === 0 || data[i+z] === 3
		|| data[i+z] === 4 || data[i+z] === 7 || data[i+z] === 9 
		|| data[i+z] === 6 || data[i+z] === 12
		|| data[i+z] === 29 || data[i+z] === 10) {
			break;
		}
...
function setConstPoolTable(data) {
	...
	var i = 10;
	var x = 1;
	var y = 105; // i + parseInt(cf.ClassFile.constant_pool_count, 10);
