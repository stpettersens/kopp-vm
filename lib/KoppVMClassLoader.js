...
var bc = require('./KoppVMBytecode.js');
 ...
 * @param {dynamic} bytecode Bytecode instruction as integer or string.
 * @returns {string} instruction Word mnemonic instruction.
*/
function lookupMnemnomicInstruction(bytecode) {
	var instruction = g.objGetKeyByValue(bc.Bytecode, bytecode);
	return instruction;
}
...
function setConstPoolTable() {
	...
	for(var i = n; i < y; i++) {
		...
		if(g.strcmp(tag, 'Methodref')) {
			...
			classContents[i+2] = 0; // Set byte to 0 to prevent re-read of byte.
			classContents[i+4] = 0;
...
function setAccessFlags() {
	var accessFlags = classContents[F_CPSIZE+3].toString(16) +
	classContents[F_CPSIZE+4].toString(16);
...
function setThisClass() {
	var thisClass = classContents[F_CPSIZE+5].toString(16) +
	classContents[F_CPSIZE+6].toString(16);
...
function setSuperClass() {
	var superClass = classContents[F_CPSIZE+7].toString(16) +
	classContents[F_CPSIZE+8].toString(16);
	console.log('setSuperClass: ' + superClass);
...
function setInterfacesCount() {
	var interfacesCount = classContents[CPSIZE+6].toString(16) +
	classContents[CPSIZE+7].toString(16);
	console.log('setInterfacesCount: ' + interfacesCount);
...
function setFieldsCount() {
	var fieldsCount = classContents[CPSIZE+7].toString(16) +
	classContents[CPSIZE+8].toString(16);
	console.log('setFieldsCount: ' + fieldsCount);
...
function setMethodsCount() {
	var methodsCount = classContents[CPSIZE+9].toString(16) +
	classContents[CPSIZE+10].toString(16);
	console.log('setMethodsCount: ' + methodsCount);
	cf.ClassFile.methods_count = methodsCount;
}
...
function trimConstPoolTable() {
	...
	for(var i = 0; i < CPSIZE.length; i++) {
		F_CPSIZE += CPSIZE[i];
	}
	console.log(F_CPSIZE);
	cf.ClassFile.cp_size = F_CPSIZE;
}
